...
public class ChangeUserPasswordResource {
    ...
    public Response changePassword(ApiRequest<ChangePasswordData> req) {
        ChangePasswordData data = (req == null) ? null : req.input;
        AuthToken token = (req == null) ? null : req.token;

        if (data == null || data.username == null || data.username.isBlank() 
                || data.newPassword == null || data.newPassword.isBlank()
                || data.oldPassword == null || data.oldPassword.isBlank()) {
                return Response.ok(
        ...
        try {
            ...
            if (!data.username.equals(session.getString("username"))) {
                return Response.ok(
            ...
            if (!account.getString("passwordHash").equals(DigestUtils.sha512Hex(data.oldPassword))) {
                return Response.ok(
                    g.toJson(ApiResponse.error(ErrorCode.INVALID_CREDENTIALS)),
                    MediaType.APPLICATION_JSON
                ).build();
            ...
            return Response.ok(
                    g.toJson(ApiResponse.success(new MessageResult("Password changed successfully"))),
                    MediaType.APPLICATION_JSON
            ).build();

        }catch (DatastoreException e) {
            LOG.log(Level.SEVERE, "Datastore error on changeuserpwd", e);
            return Response.ok(
                    g.toJson(ApiResponse.error(ErrorCode.FORBIDDEN)),
                    MediaType.APPLICATION_JSON
            ).build();
        } catch (RuntimeException e) {
            LOG.log(Level.SEVERE, "Unexpected error on changeuserpwd", e);
            return Response.ok(
                    g.toJson(ApiResponse.error(ErrorCode.FORBIDDEN)),
                    MediaType.APPLICATION_JSON
            ).build();
